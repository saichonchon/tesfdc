trigger caseRoundRobin on Case (before insert, before update) {
    ...
    for (Case cs : Trigger.new)
    {
        ...
        //--C901431--Start//
        String queueId = ((String)cs.OwnerId).substring(0,15);
        if(queueId != null && Case_Queue__c.getValues(queueId) != null && Case_Queue__c.getValues(queueId).Active__c){
            cs.Assigned_Queue__c = Case_Queue__c.getValues(queueId).Queue_Name__c;
            cs.Assigned_Queue_Time__c = datetime.now();
        }
        //--C901431--End//
    }
    ...
    for(Assignment_Groups__c[] ags : [SELECT Group_Name__c, Group_Name__r.Name, User__c, Last_Assignment__c, Millisecond__c 
    ...
    for (Integer i : queueIds.keySet())
    {
        ...
        {   
            ...
            System.debug('>>>>>Owner changed for Case ' + Trigger.new[i].Id + ' from '+Trigger.new[i].OwnerId+' to '+ ag.User__c+'--'+ag.Group_Name__r.Name);
